...
### Ports
Exposed on RunPod: 8000 (LLM), 8080 (web proxy), 18789 (OpenClaw gateway), 8642 (Hermes gateway), 22 (SSH).
...
- **Dev CLI builds**: PRs that change `a2go/` also build cross-compiled CLI binaries and attach them to a GitHub pre-release tagged `dev-{sanitized-branch}` (e.g. `dev-feat-agent-flag`). Binary version: `v0.0.0-dev.{sanitized-branch}`. Install with: `curl -sSL https://a2go.run/install.sh | bash -s -- --version dev-{sanitized-branch}`. Pre-releases are auto-deleted when the PR closes.
- The unified PR pipeline (`dev-build.yml`) orchestrates both Docker and CLI dev builds. `docker-build.yml` is a reusable workflow called by both `dev-build.yml` (PRs) and `release.yml` (production releases).
